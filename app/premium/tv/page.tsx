...
export default function PremiumTvPage() {
  ...
  const [premiumTvVideos, setPremiumTvVideos] =
    useState<PremiumTvVideo[]>([]);

  const [premiumVideoIndex, setPremiumVideoIndex] =
    useState(0);

  const [premiumVideoLoading, setPremiumVideoLoading] =
    useState(true);

  const [premiumVideoError, setPremiumVideoError] =
    useState("");
  ...
  const currentPremiumVideo =
    premiumTvVideos[premiumVideoIndex] ||
    null;
  ...
  useEffect(() => {
    ...
    async function loadPremiumTvVideos() {
      try {
        ...
        console.error(
          "Premium TV video programming error:",
          error
        );
  ...
  async function playCurrentProgram() {
    ...
    try {
      ...
      setIsPlaying(false);
      setPlaybackMessage(
        `"${currentProgram.title}" playback was interrupted. Press Play to try again.`
      );
  ...
  function previousPremiumVideo() {
    if (
      premiumTvVideos.length === 0
    ) {
      return;
    }

    const video =
      videoChannelRef.current;

    if (video) {
      video.pause();
    }

    setPremiumVideoIndex(
      (index) =>
        index <= 0
  ...
  function registerPremiumVideoPreviewStart() {
    if (
      !isPublicPreview ||
      !currentPremiumVideo
    ) {
    ...
    const previewId =
      `video:${currentPremiumVideo.mediaId}`;
    ...
    try {
      ...
      // In-memory preview protection still applies.
    }
    ...
    if (
      ...
      setPlaybackMessage(
        "This is your final free Premium TV preview program."
      );
    }
  ...
  const [tvOnAir, setTvOnAir] =
    useState<boolean | null>(null);
  const [tvBroadcastError, setTvBroadcastError] =
    useState("");
  ...
  useEffect(() => {
    ...
    async function loadTvBroadcastStatus() {
      try {
        ...
        if (!contentType.includes("application/json")) {
          throw new Error(
            "TV broadcast status returned an invalid response."
          );
        }
        ...
        if (!response.ok) {
          throw new Error(
            data.error || "TV broadcast status could not be loaded."
          );
        }
        ...
        console.error(
          "Premium TV broadcast status error:",
          error
        );
        ...
        if (!cancelled) {
          ...
          setTvBroadcastError(
            error instanceof Error
              ? error.message
              : "TV broadcast status could not be loaded."
          );
        }
      }
    }

    void loadTvBroadcastStatus();
  ...
  if (tvOnAir === null && !tvBroadcastError) {
    return (
      <main className="min-h-screen px-5 pb-32 pt-52 sm:px-8">
        <section className="mx-auto max-w-5xl rounded-[2rem] border border-cyan-300/15 bg-gradient-to-br from-cyan-500/10 via-white/[0.035] to-fuchsia-500/10 p-10 text-center shadow-2xl">
          ...
          <p className="mt-4 text-white/55">
            Confirming whether Premium TV is currently on air.
          </p>
        </section>
      </main>
    );
      ...
      <main className="min-h-screen px-5 pb-32 pt-52 sm:px-8">
        <section className="mx-auto max-w-5xl rounded-[2rem] border border-amber-300/20 bg-gradient-to-br from-amber-500/15 via-white/[0.035] to-red-500/10 p-10 text-center shadow-2xl">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-white/60">
            Premium TV cannot start until the broadcast status can be confirmed.
          </p>
  ...
  if (tvOnAir === false) {
    return (
      <main className="min-h-screen px-5 pb-32 pt-52 sm:px-8">
        <section className="mx-auto max-w-5xl overflow-hidden rounded-[2.5rem] border border-red-300/20 bg-gradient-to-br from-red-500/15 via-black/50 to-fuchsia-500/15 p-10 text-center shadow-2xl sm:p-14">
          ...
          <p className="mt-8 text-sm font-black uppercase tracking-[0.22em] text-fuchsia-300">
            SOLO BEATS ENGINE MUSIC
          </p>

          <h1 className="mt-3 text-4xl font-black sm:text-6xl">
            PREMIUM TV IS OFF AIR
          </h1>

          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-white/60">
            Premium TV is temporarily unavailable. Please check back when the channel is back on air.
          </p>
          ...
          <p className="mt-8 text-sm text-white/35">
            This page automatically checks TV status and will become available again when Premium TV is placed back ON AIR.
          </p>
        </section>
      </main>
    );
        ...
        <section className="mb-8 overflow-hidden rounded-[1.4rem] border border-fuchsia-300/20 bg-gradient-to-br from-fuchsia-700/20 via-black/70 to-cyan-500/10 sm:mb-12 sm:rounded-[2rem]">
          <div className="flex flex-col items-start gap-4 border-b border-white/10 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
            ...
            <div className="flex w-full flex-wrap gap-2 sm:w-auto">
              ...
              {currentPremiumVideo?.featured ? (
                <span className="rounded-full border border-amber-300/20 bg-amber-400/10 px-4 py-2 text-xs font-black uppercase tracking-[0.15em] text-amber-200">
                  Featured
                </span>
              ) : null}
            </div>
          ...
          {premiumVideoLoading ? (
            ...
            <div className="p-3 sm:p-8">
              ...
              <div className="mt-5 flex flex-col items-start justify-between gap-4 sm:mt-8 sm:flex-row sm:items-center">
                <div>
                  ...
                  {currentPremiumVideo.description ? (
                  ...
                  <p className="mt-2 text-xs uppercase tracking-[0.15em] text-white/35">
                    Source: {currentPremiumVideo.sourceType}
                  </p>
                </div>
            ...
            <div className="px-5 py-8 text-center sm:p-10">
              ...
              <p className="mt-2 text-sm text-white/45">
                Turn Published ON and Premium TV ON for a video in Video Manager.
              </p>
            </div>
          )}
        </section>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-white/55">
                {hasPremiumAccess
                  ? `Secondary visual music programming powered by ${premiumAlbums.length} selected albums and ${allPrograms.length} tracks.`
