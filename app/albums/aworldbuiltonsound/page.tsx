...
const tracks: Track[] = [
  { number: 1, title: "A World Built on Sound", audio: "/previews/aworldbuiltonsound.mp3" }
];
...
export default function AlbumPage() {
  ...
  const queue: PlayerTrack[] = tracks.map((track) => ({
    id: `aworldbuiltonsound-${track.number}`,
    title: track.title,
    artist: "Solo Beats",
    albumTitle: "A World Built on Sound",
    audio: track.audio,
  ...
  function playTrack(track: Track) {
    const trackId = `aworldbuiltonsound-${track.number}`;
    ...
    <main className="min-h-screen bg-black pb-36 text-white">
      <section className="mx-auto max-w-7xl px-5 py-12">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[380px_1fr] lg:items-center">
          <img src="/covers/aworldbuiltonsound.png" alt="A World Built on Sound" className="aspect-square w-full rounded-3xl object-cover shadow-2xl" />

          <div>
            <p className="text-sm font-black uppercase tracking-[0.3em] text-purple-300">Upcoming Album</p>
            <h1 className="mt-3 text-5xl font-black sm:text-6xl">A World Built on Sound</h1>
            <p className="mt-3 text-gray-400">Solo Beats â€¢ 2026 â€¢ 1 Tracks â€¢ Electronic</p>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-gray-300">A World Built on Sound is an expansive electronic experience shaped by cinematic melodies and powerful production.</p>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <button type="button" onClick={() => playTrack(tracks[0])} className="rounded-full bg-purple-600 px-7 py-3 font-black transition hover:bg-purple-500">
                {currentTrack?.id === `aworldbuiltonsound-1` && isPlaying ? "Pause Preview" : "Play Album Preview"}
              </button>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-16">
        ...
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03]">
          {tracks.map((track) => {
            const active = currentTrack?.id === `aworldbuiltonsound-${track.number}`;
            ...
            return (
              <div key={track.number} className={`grid gap-4 border-b border-white/10 p-4 last:border-b-0 sm:grid-cols-[55px_1fr_auto] sm:items-center ${active ? "bg-purple-500/10" : "hover:bg-white/[0.04]"}`}>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">A World Built on Sound â€¢ Solo Beats</p>
                </div>
